...
// Selected FX overlay for impact page: 'usd' | 'gbp' | null
let selectedFx = 'usd';

// Annual average EUR/USD exchange rate (1999-2025)
const eurusdAnnual = {
  '1999': 1.07,
  '2000': 0.92,
  '2001': 0.90,
...
// Annual average EUR/GBP exchange rate (1999-2025)
const eurgbpAnnual = {
  '1999': 0.66,
  '2000': 0.61,
  '2001': 0.62,
...
// Annual average EUR/JPY exchange rate (1999-2025)
const eurjpyAnnual = {
  '1999': 121.19,
  '2000': 99.50,
  '2001': 108.81,
...
// Annual average Eurozone inflation (%)
...
function renderCombinedRatesChart() {
  ...
  // Toggle highlight of explanation boxes based on selection
  if (isImpactPage) {
    const fxBox = document.getElementById('fxExplanationBox');
    const macroBox = document.getElementById('macroExplanationBox');
    if (fxBox && macroBox) {
      const isCurrency = selectedFx === 'usd' || selectedFx === 'gbp' || selectedFx === 'jpy';
      const isMacro = selectedFx === 'infl' || selectedFx === 'gdp';
      fxBox.classList.toggle('explain-highlight', isCurrency);
      macroBox.classList.toggle('explain-highlight', isMacro);
    }
  }
}
...
function renderCombinedRatesChart(context = false) {
  ...
  const isImpactPage = typeof window !== 'undefined' && window.location && window.location.pathname.includes('impact.html');
  const eurusdData = labels.map(lab => {
    const yr = lab.substring(0, 4);
    return eurusdAnnual[yr] !== undefined ? eurusdAnnual[yr] : null;
  });
  const eurgbpData = labels.map(lab => {
    const yr = lab.substring(0, 4);
    return eurgbpAnnual[yr] !== undefined ? eurgbpAnnual[yr] : null;
  });
  const eurjpyData = labels.map(lab => {
    const yr = lab.substring(0, 4);
    return eurjpyAnnual[yr] !== undefined ? eurjpyAnnual[yr] : null;
  });
  const inflData = labels.map(lab => {
    const yr = lab.substring(0, 4);
    return inflationAnnual[yr] !== undefined ? inflationAnnual[yr] : null;
  });
  const gdpData = labels.map(lab => {
    const yr = lab.substring(0, 4);
    return gdpAnnual[yr] !== undefined ? gdpAnnual[yr] : null;
  });
  ...
  if (isImpactPage) {
    ...
    } else if (selectedFx === 'gbp') {
      datasets.push({
        label: 'EUR/GBP (annual avg)',
        data: eurgbpData,
        yAxisID: 'y2',
        borderColor: '#ff7f0e',
    ...
    } else if (selectedFx === 'jpy') {
      datasets.push({
        label: 'EUR/JPY (annual avg)',
        data: eurjpyData,
        yAxisID: 'y2',
        borderColor: '#20c997',
    ...
    } else if (selectedFx === 'infl') {
      datasets.push({
        label: 'Eurozone Inflation (annual %)',
        data: inflData,
        yAxisID: 'y2',
        borderColor: '#e83e8c',
        backgroundColor: 'rgba(232,62,140,0.10)',
        borderWidth: 2,
        fill: false,
        tension: 0.35,
        pointRadius: 2,
    ...
    } else if (selectedFx === 'gdp') {
      datasets.push({
        label: 'EU GDP Growth (annual %)',
        data: gdpData,
        yAxisID: 'y2',
        borderColor: '#0d6efd',
  ...
  if (isImpactPage) {
    const axisLabel = selectedFx === 'gbp' ? 'EUR/GBP' : (selectedFx === 'jpy' ? 'EUR/JPY' : (selectedFx === 'infl' ? 'Inflation (%)' : (selectedFx === 'gdp' ? 'GDP growth (%)' : 'EUR/USD')));
    scales.y2 = {
      position: 'right',
      beginAtZero: false,
      title: { display: true, text: axisLabel, font: { size: 15 } },
      grid: { drawOnChartArea: false },
      ticks: { callback: v => ((selectedFx === 'infl' || selectedFx === 'gdp') ? v.toFixed(1) + '%' : v.toFixed(2)) }
    };
  }
